...
import { ProductModel, ReviewModel, SpecificationsModel, KnowledgeBaseModel, VariantModel, AIContextTextModel, InventoryModel } from '@/models';
...
export const deleteProduct = async (req: Request, res: Response) => {
  try {
    ...
    await Promise.all([
      ...
      AIContextTextModel.deleteMany({ product_id }),
  ...
  try {
    ...
    for (const productData of data) {
      try {
        ...
        // 4. Create AI context if it exists
        if (ai_context_text) {
          try {
            ...
            results.errors.push(`Failed to create AI context for product ${product_id}: ${aiError}`);
          }
        }

        // 5. Create specifications if they exist
        if (specifications) {
          try {
        ...
        // 6. Create variants if they exist
